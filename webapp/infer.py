...
class Predictor:
    ...
    def overlay_predictions(self, image, predictions, confidence_threshold=0.5):
        ...
        for box, score, cls in zip(filtered_boxes, filtered_scores, filtered_classes):
            ...
            # Measure label size so we can do edge checks
            (label_w, label_h), baseline = cv2.getTextSize(
                label, cv2.FONT_HERSHEY_SIMPLEX, font_scale, thickness
            )

            # Flip label below the box if there isn't enough room above it
            if int(y1) - label_h - baseline < 0:
                label_y = int(y2) + label_h + baseline  # below bottom edge
            else:
                label_y = int(y1) - baseline  # above top edge (default)

            # Clamp label horizontally so it doesn't run off the right edge
            label_x = min(int(x1), width - label_w)
    ...
    def infer(self, image, details=True):
        ...
        image_with_overlay = self.overlay_predictions(
            image, predictions, confidence_threshold=threshold
        )
        if details:
    ...
    def convert_predictions_to_json(self, predictions):
        ...
        boxes = (
            ...
            and hasattr(instances.pred_boxes, "tensor")
            else None
        )
        scores = (
            instances.scores.cpu().numpy() if hasattr(instances, "scores") else None
        )
        classes = (
